...
function getAllReminders(token: string = "") {
  return request(app).get("/api/reminders").set("X-Authorization", token);
}
...
afterAll(() => {
  ...
  if (totalPoints !== maxPoints) {
    console.error(
      `Something went wrong D:! Total Points: ${totalPoints} !== Max Points: ${maxPoints}`
    );
  }
  ...
  console.log(
    `Nota ${(studentPoints < 0.51 * maxPoints
      ? ((4 - 1) * studentPoints) / (0.51 * maxPoints) + 1
      : ((7 - 4) * (studentPoints - 0.51 * maxPoints)) /
          (maxPoints * (1 - 0.51)) +
        4
    ).toFixed(1)}`
  );
...
describe("POST /api/auth/login", async (context) => {
  ...
  test("Devuelve código de estado 401 cuando el usuario no existe o contraseña incorrecta", async () => {
    const { status: loginStatus1 } = await loginRequest(
      "notadmin",
      "certamen123"
    );
    const { status: loginStatus2 } = await loginRequest(
      "admin",
      "certamen1234"
    );
...
describe("POST /api/reminders", async () => {
  ...
  test("Crea un recordatorio correctamente", async ({ token }) => {
    const { body: createdItem1, status: statusItem1 } =
      await createReminderRequest("Item 1", true, token);
  ...
  test("Crea un recordatorio correctamente con importante default", async ({
    token,
  }) => {
    const { body: createdItem1, status: statusItem1 } =
      await createReminderRequest("Item 2", undefined, token);
  ...
  test("Devuelve código de estado 400 cuando no se envían los datos apropiados", async ({
    ...
    const { status: statusItem3 } = await createReminderRequest(
      false,
      true,
      token
    );
    const { status: statusItem4 } = await createReminderRequest(
      "Item 4",
      26,
      token
    );
  ...
  test("Persiste el item creado previamente", async ({ token }) => {
    const { body: createdItem5 } = await createReminderRequest(
      "Item 5",
      undefined,
      token
    );
    ...
    expect
      .soft(reminders?.find((reminder) => reminder.id === createdItem5.id))
      .toMatchObject({
        content: "Item 5",
      });
  });
...
describe("PATCH /api/reminders/[id]", () => {
  ...
  test("Actualiza un item correctamente", async ({ token }) => {
    const { body: createdItem6 } = await createReminderRequest(
      "Item 6",
      true,
      token
    );
    ...
    const { body: updatedItem1, status: statusUpdatedItem1 } =
      await updateItemRequest(createdItem6.id, firstUpdate, token);

    expect
      .soft(statusUpdatedItem1, "Dar un código 200 al actualizar el título")
      .toBe(200);
    expect
      .soft(updatedItem1, "El objeto retornado tiene el título actualizado")
      .toMatchObject(firstUpdate);
    ...
    const { body: updatedItem2, status: statusUpdatedItem2 } =
      await updateItemRequest(createdItem6.id, secondUpdate, token);

    expect
      .soft(
        statusUpdatedItem2,
        "Dar un código 200 al actualizar el título y el estado"
      )
      .toBe(200);
    expect
      .soft(
        updatedItem2,
        "El objeto retornado tiene el título y el estado actualizado"
      )
      .toMatchObject(secondUpdate);

    const { body: reminders } = await getAllReminders(token);

    expect
      .soft(reminders?.find((reminder) => reminder.id === updatedItem2.id))
      .toMatchObject(secondUpdate);
  });
  ...
  test("Devuelve código de estado 400 cuando no se envían los datos apropiados", async ({
    ...
    const { body: createdItem7 } = await createReminderRequest(
      "Item 7",
      false,
      token
    );

    const { status: updateStatus1 } = await updateItemRequest(
      createdItem7.id,
      { content: 123 },
      token
    );
    ...
    const { status: updateStatus2 } = await updateItemRequest(
      createdItem7.id,
  ...
  test("Devuelve código de estado 404 cuando el ítem a actualizar no existe", async ({
    token,
  }) => {
    const { status: updateStatus1 } = await updateItemRequest(
      randomUUID(),
...
describe("DELETE /api/reminders/[id]", () => {
  ...
  test("Borra un item correctamente", async ({ token }) => {
    ...
    const { body: deletedItem1, status: deleteStatus1 } = await deleteRequest(
      createdItem5.id,
      token
    );
  ...
  test("Devuelve código de estado 404 cuando el ítem a borrar no existe", async ({
    token,
  }) => {
...
test("Los endpoints indicados están protegidos por un token de autenticación", async ({
  token,
}) => {
  const { status: listStatus } = await getAllReminders();
  const { status: createStatus } = await createReminderRequest("ITEM");
  const { status: updateStatus } = await updateItemRequest(randomUUID(), {
    content: "UPDAte",
  });
...
describe("GET /api/reminders", async () => {
  ...
  test("Lista recordatorios en el orden indicado", async ({ token }) => {
    ...
    expect
      .soft(JSON.stringify(reminders))
      .toBe(JSON.stringify(sortedReminders));
    expect
      .soft(JSON.stringify(reminders))
      .toBe(JSON.stringify(sortedReminders)); // two points
  });
});
